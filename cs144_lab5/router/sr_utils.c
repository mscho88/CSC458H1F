...
uint16_t tcp_cksum(const void *packet, int len){
    ...
    tcp_hdr->checksum = 0;
    unsigned char *buf = malloc(ntohs(ip_hdr->ip_len) - sizeof(sr_ip_hdr_t) + sizeof(sr_tcp_pseudo_hdr_t));
    ...
    uint16_t checksum = cksum(buf, ntohs(ip_hdr->ip_len) - sizeof(sr_ip_hdr_t) + sizeof(sr_tcp_pseudo_hdr_t));
